function uploadImage(event){
   ...
   if (files && files.length > 0) {
      if (files.length > 1) {
         infoImage.innerHTML = `<img src="./assets/images/icon-info.svg" alt=""> Não é possível enviar mais de uma imagem.`
      ...
      if(file.size > 500000){
         infoImage.innerHTML = `<img src="./assets/images/icon-info.svg" alt=""> Arquivo é muito extenso. Envie um arquivo de até 500KB.`
         infoImage.classList.add('error-text')
         infoImage.classList.remove('info-image')
         return
      }
      ...
      if (file.type.startsWith("image/png") || file.type.startsWith("image/jpeg") && file.size <= 500000) {
         ...
         avatar.innerHTML = `
            <img src="${imageUrl}" alt="" id="uploaded-image">
         ...
         infoImage.innerHTML = `<img src="./assets/images/icon-info.svg" alt=""> Adicione sua foto (JPG ou PNG, tam. máximo: 500KB).`
...
form.addEventListener('submit', (e) => {
   ...
   if(email == ""){
      ...
      errorEmail.innerHTML = `<img src="./assets/images/icon-info.svg" alt="">Insira um email válido`
   } else {
   ...
   if(githubUsername == "" || !githubUsername.startsWith("@")){
      ...
      errorGithub.innerHTML = `<img src="./assets/images/icon-info.svg" alt="">Adicione um nome de usuário do Github válido`
   } else {
   ...
   if(fullName != "" && email != "" && githubUsername != "" && githubUsername.startsWith("@") && imageUrl != ""){
      ...
      main.innerHTML = `
      <img src="./assets/images/logo-full.svg" alt="" id="logo">
      <div id="div-ticket" class="container-desktop">
         <h1>Parabéns, <span class="user-info">${fullName}</span>! Seu ingresso está pronto!.</h1>
   
         <p id="ticket-email">O ingresso foi enviado para <span class="user-info">${email}</span>, novidades serão comunicadas via email!.</p>
